...
function App() {
  ...
  // Tabs + Submitted Issues view mode
  const [activeTab, setActiveTab] = React.useState('check'); // 'check' | 'report' | 'submitted'
  const [submittedViewMode, setSubmittedViewMode] = React.useState('grid'); // 'grid' | 'single'
  ...
  // Keep currentIndex in range when issues change
  React.useEffect(() => {
    if (issues.length === 0) {
      setCurrentIndex(0);
  ...
  return (
    <div>
      ...
      {/* Tabs */}
      <nav className="tabs">
        {[{ key: 'check', label: 'Check Status', icon: '🔎' }, { key: 'report', label: 'Submit Issue', icon: '📝' }, { key: 'submitted', label: 'Submitted Issues', icon: '📋' }].map(t => (
      ...
      {activeTab === 'check' && (
      <div style={{ background: '#23242a', borderRadius: 20, boxShadow: '0 4px 16px #0003', padding: 24, margin: '0 24px 12px 24px', border: '1px solid #333', transition: 'box-shadow 0.3s, border-radius 0.3s' }}>
        <h2 style={{ color: '#90caf9', fontSize: 18, marginBottom: 8, textAlign: 'center', fontWeight: 600 }}>Check Issue Status / Feedback</h2>
        ...
        <div style={{ padding: '32px 24px 24px 24px', margin: '0 24px', background: '#23242a', borderRadius: 20, boxShadow: '0 4px 16px #0003', border: '1px solid #333', transition: 'box-shadow 0.3s, border-radius 0.3s' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, background: '#23242a', borderRadius: 16, boxShadow: '0 4px 16px #0003', padding: 20, border: '1px solid #333', transition: 'box-shadow 0.3s, border-radius 0.3s' }}>
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="Enter your email address" style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #444', marginTop: 4, background: '#181a20', color: '#e0e0e0' }} />
            </div>
            <div>
              <label style={{ fontWeight: 'bold', color: '#90caf9' }}>State</label>
              <select value={state} onChange={e => setState(e.target.value)} required style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #444', marginTop: 4, background: '#181a20', color: '#e0e0e0' }}>
            ...
            <div>
              ...
              <input type="text" value={city} onChange={e => setCity(e.target.value)} required placeholder="Enter your city" style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #444', marginTop: 4, background: '#181a20', color: '#e0e0e0' }} />
            </div>
            <div>
              <label style={{ fontWeight: 'bold', color: '#90caf9' }}>Category</label>
              <select value={category} onChange={e => { setCategory(e.target.value); setDepartment(categoryToDepartment[e.target.value] || ''); }} required style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #444', marginTop: 4, background: '#181a20', color: '#e0e0e0' }}>
            ...
            <div>
              ...
              {!voiceEnabled && (
                <textarea value={description} onChange={e => setDescription(e.target.value)} required rows={3} style={{ width: '100%', padding: 10, borderRadius: 10, border: '1.5px solid #1976d2', marginTop: 10, background: '#181a20', color: '#e0e0e0', fontSize: 16, boxShadow: '0 2px 8px #1976d220' }} placeholder="Describe the issue..." />
              )}
              {voiceEnabled && (
                <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', background: '#23242a', borderRadius: 12, border: '1.5px solid #1976d2', padding: 18, boxShadow: '0 2px 12px #1976d220', minHeight: 90, position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button type="button" onClick={handleStartListening} disabled={listening || !(window.SpeechRecognition || window.webkitSpeechRecognition)} style={{ background: listening ? '#bdbdbd' : '#1976d2', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontWeight: 'bold', fontSize: 16, cursor: listening ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px #1976d220', transition: 'background 0.2s' }} title={!(window.SpeechRecognition || window.webkitSpeechRecognition) ? 'Voice input not supported in this browser' : listening ? 'Listening...' : 'Start voice input'}>🎤 Speak</button>
                    <button type="button" onClick={handleStopListening} disabled={!listening} style={{ background: !listening ? '#bdbdbd' : '#f44336', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontWeight: 'bold', fontSize: 16, cursor: !listening ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px #f4433620', transition: 'background 0.2s' }}>⏹ Stop</button>
                  </div>
            ...
            <div>
              ...
              <input type="file" accept="image/*" onChange={e => setPhoto(e.target.files[0])} style={{ marginTop: 4, color: '#e0e0e0', background: '#23242a', border: 'none', borderRadius: 6, padding: '8px 0', fontSize: 15, boxShadow: '0 1px 4px #0002', cursor: 'pointer', width: '100%' }} />
            </div>
            <div>
              ...
              <button type="button" onClick={handleDetectLocation} disabled={autoLocating} style={{ background: '#1976d2', color: '#fff', padding: '8px 12px', border: 'none', borderRadius: 6, fontWeight: 'bold', cursor: autoLocating ? 'not-allowed' : 'pointer', marginTop: 10, width: '100%' }}>{autoLocating ? 'Detecting...' : 'Auto Detect'}</button>
                ...
                (() => {
                  ...
                  return (
                    <div style={{ marginTop: 12, textAlign: 'center' }}>
                      <iframe title="Detected Location" width="100%" height="220" frameBorder="0" style={{ borderRadius: 8, border: '1px solid #333' }} src={mapUrl} allowFullScreen></iframe>
                      <div style={{ fontSize: 13, color: '#bdbdbd', marginTop: 4 }}>Map preview (OpenStreetMap)</div>
                    </div>
                  );
                })()
              )}
            </div>
            <button type="submit" disabled={loading} style={{ background: '#1976d2', color: '#fff', padding: '10px 0', border: 'none', borderRadius: 6, fontWeight: 'bold', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px #0002' }}>{loading ? 'Submitting...' : 'Submit Issue'}</button>
          </form>
      ...
      {activeTab === 'submitted' && (
        <div style={{ margin: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: 12, background: '#181a20', borderRadius: 20, boxShadow: '0 2px 12px #0004', border: '1px solid #333' }}>
            <h2 style={{ color: '#90caf9', fontSize: 20, margin: '4px 0 10px', textAlign: 'center', fontWeight: 700 }}>Issue Heat Map</h2>
            <div style={{ height: 320, width: '100%', borderRadius: 16, overflow: 'hidden', margin: '0 auto', maxWidth: 540 }}>
              <MapContainer center={[22.9734, 78.6569]} zoom={5} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false} dragging={true}>
                ...
                {issues.map((issue, i) => {
                  ...
                  return (
                    <Circle key={i} center={coords} radius={4000} pathOptions={{ color: '#f44336', fillColor: '#f44336', fillOpacity: 0.4 }} />
                  );
                })}
              </MapContainer>
            </div>
            <div style={{ textAlign: 'center', color: '#bdbdbd', fontSize: 13, marginTop: 6 }}>Red circles show reported issue locations (approximate heat map)</div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <h2 className="section-title" style={{ margin: 0 }}>Submitted Issues</h2>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setSubmittedViewMode('grid')} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid ' + (submittedViewMode === 'grid' ? '#1976d2' : '#333'), background: submittedViewMode === 'grid' ? '#0f1623' : '#181a20', color: submittedViewMode === 'grid' ? '#90caf9' : '#ddd', fontWeight: 700 }}>Grid</button>
              <button onClick={() => setSubmittedViewMode('single')} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid ' + (submittedViewMode === 'single' ? '#1976d2' : '#333'), background: submittedViewMode === 'single' ? '#0f1623' : '#181a20', color: submittedViewMode === 'single' ? '#90caf9' : '#ddd', fontWeight: 700 }}>One at a time</button>
            </div>
          </div>

          {issues.length === 0 ? (
            <p className="center dim">No issues reported yet.</p>
          ) : submittedViewMode === 'grid' ? (
            <section className="card fade-in" style={{ marginTop: 0 }}>
                    ...
                    <article key={issue._id} className={`gallery-card border-${statusClass}`}>
                      ...
                      <div className="g-body">
                        ...
                        <div className="g-tags">
                          ...
                          <button type="button" className="copy-btn" onClick={() => navigator.clipboard?.writeText(issue._id)} aria-label="Copy Issue ID">📋</button>
                        </div>
            ...
            <section className="card fade-in" style={{ marginTop: 0, padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <button onClick={() => setCurrentIndex(i => Math.max(0, i - 1))} disabled={currentIndex === 0} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #333', background: currentIndex === 0 ? '#1f2229' : '#181a20', color: '#ddd', fontWeight: 600 }}>← Prev</button>
                <div style={{ color: '#90caf9', fontWeight: 700 }}>{issues.length ? (currentIndex + 1) : 0} / {issues.length}</div>
                <button onClick={() => setCurrentIndex(i => Math.min(issues.length - 1, i + 1))} disabled={currentIndex >= issues.length - 1} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #333', background: currentIndex >= issues.length - 1 ? '#1f2229' : '#181a20', color: '#ddd', fontWeight: 600 }}>Next →</button>
              </div>
              {issues[currentIndex] && (() => {
                const issue = issues[currentIndex];
                const statusClass = issue.status === 'solved' ? 'green' : issue.status === 'underwork' ? 'amber' : issue.status === 'not solved' ? 'red' : 'blue';
                return (
                  <article key={issue._id} className={`gallery-card border-${statusClass}`} style={{ maxWidth: 640, margin: '0 auto' }}>
                    {issue.photoUrl && (
                      <div className="g-media-wrapper">
                        <img src={`${API_BASE}${issue.photoUrl}`} alt={issue.description} className="g-photo" />
                        <span className={`g-badge status ${issue.status.replace(' ', '')}`}>{issue.status}</span>
                    ...
                    <div className="g-body">
                      ...
                      <div className="g-tags">
                        <span className="g-tag cat">{issue.category || '—'}</span>
                        <span className="g-tag dept">{issue.department ? `Dept: ${issue.department}` : ''}</span>
                        <span className="g-tag id" title={issue._id}>{issue._id}</span>
                        <button type="button" className="copy-btn" onClick={() => navigator.clipboard?.writeText(issue._id)} aria-label="Copy Issue ID">📋</button>
                      </div>
                      <div className="g-meta"><span className="label">Location:</span>{issue.location}</div>
                      <div className="g-meta"><span className="label">Department:</span> {issue.department || '—'}</div>
